    ...
    void Draw() {
        ...
        for (int i=0; i<mesh.triangles.size(); i++) {
            ...
            for (int j = 0; j<gl_Position.length; j++) {
                println("clip:", gl_Position[j].x, gl_Position[j].y, gl_Position[j].z, gl_Position[j].w);
                s_Position[j] = gl_Position[j].homogenized();
                Vector3 p= s_Position[j];
                println("NDC", p.x, p.y, p.z);
            }
